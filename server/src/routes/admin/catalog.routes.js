...
r.post('/products/:id/images',requirePermission('products.update'),productUpload.fields([{name:'cover',maxCount:1},{name:'images',maxCount:12}]),asyncHandler(async(req,res)=>{
  ...
  for(let i=0;i<extras.length;i++){const url=await optimize(extras[i]);const primary=!hasPrimary&&!cover&&i===0;const x=await query('INSERT INTO product_images(product_id,image_url,alt_text,is_primary,sort_order) VALUES(?,?,?,?,?)',[req.params.id,url,req.body.altText||'',primary?1:0,start+i+1]);made.push({id:x.insertId,image_url:url,is_primary:primary?1:0})}
