...
r.get('/track-order', asyncHandler(async (req, res) => {
  const rows = await query(`SELECT o.id,o.order_number,o.status,o.payment_status,o.tracking_code,o.courier_name,o.created_at FROM orders o JOIN users u ON u.id=o.user_id WHERE o.order_number=? AND (LOWER(u.email)=LOWER(?) OR u.phone=?)`, [req.query.orderNumber, req.query.contact, req.query.contact]);
