...
// cPanel commonly provisions databases before app setup.
try{await c.query('USE `'+env.db.name.replaceAll('`','``')+'`')}catch(e){if(e.code!=='ER_BAD_DB_ERROR')throw e;
...
// Medico full-upgrade columns. Existing data is preserved.
for (const [table,column,definition] of [
  ['prescriptions','deleted_at','DATETIME NULL'],
  ['prescriptions','original_name','VARCHAR(255) NULL'],
  ['prescriptions','mime_type','VARCHAR(120) NULL'],
  ['prescriptions','file_size','BIGINT UNSIGNED NULL'],
  ['orders','cancel_reason','VARCHAR(500) NULL'],
  ['orders','cancelled_at','DATETIME NULL'],
  ['orders','hidden_by_user','TINYINT(1) DEFAULT 0']
]) await ensureColumn(table,column,definition);

const medicoMigration = fs.readFileSync(new URL('../../../database/migrations/002_medico_full_upgrade.sql',import.meta.url),'utf8');
await c.query(medicoMigration);

await c.query(fs.readFileSync(new URL('../../../database/migrations/003_integrations.sql',import.meta.url),'utf8'));
await c.query(fs.readFileSync(new URL('../../../database/migrations/004_mailbox_and_images.sql',import.meta.url),'utf8'));
